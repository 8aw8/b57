...
begin
 if not(ComboBox1.ItemIndex=-1) then begin
  ...
  Sql.Clear;
  SQL.Add(' select c.creator_name as "Завод-изготовитель", n.price as "Стоимость", n.pricedate as "Дата формирования цены", c.creator_id, n.product_id ');
...
begin
    ...
    try
     ...
     else ShowMessage('Выбирете изделие');
 end;
...
begin
 ...
 else ShowMessage('Выбирите изменяемую цену.');
end;
